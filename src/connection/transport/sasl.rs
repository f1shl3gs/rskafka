...
use rsasl::property::{AuthzId, OAuthBearerKV, OAuthBearerToken};
...
pub struct OauthBearerCredentials {
    ...
    /// long enough to complete authentication. This should cache the token and only
    /// request a new one when the old is close to expiring.
    ///
    /// The Token must be on [RFC 6750](https://www.rfc-editor.org/rfc/rfc6750) format.
    pub callback: OauthCallback,

    /// ID of a user to impersonate. Can be left as `None` to authenticate using the
    /// user for the token returned by `callback`.
    pub authz_id: Option<String>,

    /// Custom key-value pairs sent as part of the SASL request. Most normal usage can
    /// let this be an empty list.
